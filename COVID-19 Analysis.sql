...
-- Vaccine Doses Administered per Country
 
 SELECT DISTINCT vac.location, SUM(Cast(vac.new_vaccinations as bigint))OVER (PARTITION BY vac.location) AS VaccineDosesAdministered
 FROM PortfolioProject.dbo.CovidVaccinations vac
 WHERE vac.continent is not null
 AND vac.location not in ('World', 'European Union', 'International', 'Upper middle income', 'High income', 'Lower middle income', 'Low income')
 GROUP BY vac.location, vac.new_vaccinations
 ORDER BY VaccineDosesAdministered desc
